...
public abstract class HotelBooking extends UserDetails {
	...
	private double calculateTotalPrice(String doYouWantAdditionalMeal,int numOfAdults,int numOfChildren,int numOfStayDays, double roomCharges) {
	...
	protected void displayHotelFinalPriceByAddingAllPrices(HotelBooking[] hotelBooking, String roomSelected, int numberOfAdults,
			int numberOfChildren, double roomCharges, int numOfStayDays) {
		System.out.println(
		...
		for (int i = 0; i < hotelBooking.length; i++) {
			if (roomSelected.equalsIgnoreCase(hotelBooking[i].roomType)) {
				hotelBooking[i].calculateTotalPrice(doYouWantAdditionalMeal,numberOfAdults,numberOfChildren, numOfStayDays,roomCharges);
	...
	private void displayHotelPackageDetails(String doYouWantAdditionalMeal, int numOfAdults, int numOfChildren,
		...
		if (doYouWantAdditionalMeal.equalsIgnoreCase("Y")) {
			System.out.println(
					"Lunch Pre-Booked Special Rate Adults	@ " + numberOfAdults + "	:$" + (numOfAdults * numOfStayDays * 25));
			System.out.println(
					"Lunch Pre-Booked Special Rate Children	@ " + numberOfChildren + "	:$" + (numOfChildren * numOfStayDays * 5));
		}
